...
cleaning.py
-----------
Downloads the Genius lyrics dataset and produces two clean output files:

1. clean_genre_lyrics.csv  – genre classification
   Columns : tag, lyrics
   Range   : 1923–2023, English only, uniform distribution across genres

2. clean_hit_lyrics.csv    – hit prediction
   Columns : title, artist, title_clean, artist_clean, lyrics, rank, top_10
...
RUN_GENRE_CLEANING = False  # Set True to re-run genre dataset build
RUN_HIT_CLEANING   = False  # Set True to re-run hit dataset build
...
def _download_raw_df(usecols: list[str]) -> pd.DataFrame:
    """
    Downloads the Kaggle dataset and returns a DataFrame filtered to
    English songs with non-null lyrics.
    """
...
def build_genre_dataset() -> None:
    ...
    Steps:
    - Download Genius dataset (1923–2023), English only
    - Drop 'misc' tag and rows without a tag
    - Undersample to uniform distribution across genres
...
def build_hit_dataset() -> None:
    ...
    Steps:
    - Download Genius dataset (2003–2023), English only
    - Merge with Billboard chart data on normalised title + artist
    - Save columns: title, artist, title_clean, artist_clean, lyrics, rank, top_10
    """
    ...
    df = df[["title", "artist", "title_clean", "artist_clean", "lyrics", "rank", "top_10"]]
...
if __name__ == "__main__":
    if RUN_GENRE_CLEANING:
        build_genre_dataset()
    else:
        print("Genre cleaning skipped (RUN_GENRE_CLEANING=False).")

    if RUN_HIT_CLEANING:
        build_hit_dataset()
    else:
        print("Hit cleaning skipped (RUN_HIT_CLEANING=False).")